...
2)  パッケージはできるだけ文字のあるドライブ（C ドライブなど）のライブラリに保存してください。パスが 「\\」 で始まるパッケージライブラリは、できるだけ使用しないでください。\
3)  **rmarkdown** [パッケージは、「\\」で始まるパッケージライブラリに保存しないでください。]{.ul}「\\」で始まるパッケージライブラリに保存した場合、TinyTex や Pandoc に接続できなくなります。
...
まず、パッケージがどこにインストールされているかを知りたい場合は、以下のコードを実行するとパッケージライブラリのパスを確認できます。R がパッケージのインストール・ロード・検索に使用している順序で、パスが表示されます。他のデフォルトライブラリを使用したい場合は、パスの順序を変更することができます（以下を参照）。
