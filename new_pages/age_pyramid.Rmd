...
この方法では、年齢<u>カテゴリ</u>である `age_cat5` の列ではなく、年齢の<u>数値型</u>列を使用します。そこで、この年齢の数値型列の型が本当に数値型であることを確認します。
...
* `ggplot()` 内で、年齢の数値型列を使用して **2 つの**ヒストグラムを作成します。2 つのグループ化された値（ここでは、性別の男性と女性）それぞれについて 1 つずつ作成します。これを行うには、性別ごとのヒストグラムのデータを `geom_histogram()` コマンドの中で指定し、それぞれのフィルターを `linelist` に適用します。
...
pyramid_data <- linelist %>%
  ...
  mutate(percent = round(100*(counts / sum(counts, na.rm=T)), digits = 1), 
         percent = case_when(
            ...
            TRUE          ~ NA_real_))    # NA 値の型も数値型でなければならない
```
