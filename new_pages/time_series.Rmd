...
Note that the first four weeks of our data in the lagged temperature variable 
are missing (`NA`) - as there are not four weeks prior to get data from. So we 
have to drop these missings and store as a new dataset called `lag_counts`. 
...
## fit your model using the counts dataset (without missings in first four weeks)
fitted_model <- trending::fit(model, lag_counts)
...
forecasts <- purrr::map(unique(case_roll$.id), 
  ...
  # forecast with data want to predict with 
  forecasts <- fitted_model %>% 
    predict(pred_data %>% select(-case_int)) %>% 
