...
<!-- ======================================================= -->
## Colour schemes {#ggplot_tips_colors}

One thing that can initially be difficult to understand with **ggplot2** is control of colour schemes.  

To control "color" of *plot objects* (geoms/shapes like points, bars, lines, etc) you will be adjusting either the `color = ` aesthetic (the *exterior* color) or the `fill = ` aesthetic (the *interior* color). The only exception to the this behavior is `geom_point()`, where you really only get to control `color = `, which controls the color of the point (interior and exterior).  

When setting colours you can use colour names recognized by R like `"red"` (see [complete list](http://sape.inf.usi.ch/quick-reference/ggplot2/colour) or enter `?colors`), or a specific hex colour such as `"#ff0505"`.
...
As explained the [ggplot basics] section on [mapping data to the plot](#ggplot_basics_mapping), aesthetics such as `fill = ` and `color = ` can be defined either *outside* of a `mapping = aes()` statement or *inside* of one. If *outside* the `aes()`, the assigned value (e.g. `color = "blue"`) will apply for *all* data plotted by the geom. If *inside*, the aesthetic should be mapped to a column, like `color = hospital`, and the expression of a given observation will vary by the value for that row in the data. A few examples:  
...
Once you map a column toa plot aesthetic (e.g. `x = `, `y = `, `fill = `, `color = `...), your plot will gain a scale/legend. See above how the scale can be continuous, discrete, date, etc. values depending on the class of the assigned column. If you have multiple aesthetics mapped to columns, your plot will have multiple scales.  
...
2) The second part, the AESTHETIC, should be the aesthetic that you want to adjust the scale for (fill, shape, color, size, alpha...) - the options here also include x and y.  
...
One of the most useful tricks is using "manual" scaling functions to explicitly assign colors as you descire. These are functions with the syntax `scale_xxx_manual()` (e.g. `scale_colour_manual()` or `scale_fill_manual()`). Each of the below arguments are demonstrated in the code example below.  
...
Continuous scales can involve additional nuance. The defaults are usually quite pleasing, but you may want to adjust the values, cutoffs, etc.  
