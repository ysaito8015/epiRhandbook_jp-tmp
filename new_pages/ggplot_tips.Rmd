...
**ggplot2** を使う上で最初に躓きやすいものが色の調整です。この節では点や線、棒グラフやタイルなど、図のオブジェクトの色の調整を扱います。テキストやタイトル、背景などの色の調整に関しては [ggplot の基礎](#ggplot-basics)のページの[テーマ](#ggplot_basics_themes)の節を参照してください。
...
前章 [ggplot の基礎](#ggplot-basics)の[データを図にマップする](#ggplot_basics_mapping)のページで示した通り、`fill = ` や `color = ` などのエステティクスは `mapping = aes()` の<u>外側</u>と<u>内側</u>どちらからでも定義できます。`aes()` の外側で定義する場合、値は定数でなければならず（例 `color = "blue"`）、描画される全てのデータポイントに適応されます。内側から定義する場合、エステティクスは `color = hospital` のようにデータフレームの列に対応され、描画のされ方は各行の値に依存します。以下にいくつかの例を示します：  
...
# バープロットのfillを離散数からなる列、colorを単一色で指定する
...
列をエステティクスに対応させると（例 `x = `、`y = `、`fill = `、`color = `...）、 図にスケールとレジェンドが加わります。上記の例を見ると、スケールが連続数、離散数、日付など、スケールが対応する列のクラスによって変化する様子が分かると思います。列に対応するエステティクスが複数あるときは、スケールもその数だけ存在します。

スケールは一連の `scales_()` 関数を使うことで操作することができます。これらの関数は `scale_AESTHETIC_METHOD()` のように、3 つの部分からなります：  

1) 最初の部分 `scale_()` は共通です。 
2) 2 つ目の部分 AESTHETIC　はスケールを調整したいエステティクス（`_fill_`、`_shape_`、`_color_`、`_size_`、`_alpha_`...) が入ります - ここには `_x_` や `_y_` も含まれます。  
3) 最後に来る METHOD は列のクラスまたは目的に応じて `_discrete()`、`continuous()`、`_date()`、`_gradient()`、`_manual()` などが入ります。その他のオプションもありますが、ここに挙げたものが最もよく使われます。  
...
離散数のスケールでは、レベルが表示される順序を `breaks = `、値の表示を `labels = `で操作できます。以下の例のように、各引数に文字ベクターを指定してください。また、`NA` は `na.translate = FALSE` で簡単に除外することができます。

日付スケールについては[流行曲線](#epidemic-curves)のページでより詳しく解説しています。


### マニュアル調整 {.unnumbered}  

"マニュアル" スケール関数を用いることで、色を具体的に指定することができます。これらの関数は `scale_xxx_manual()` の形（例：`scale_colour_manual()` や `scale_fill_manual()`）を取ります。例を通して下に挙げた引数の使用法をそれぞれ見ていきましょう。

* `values = `により色を指定します。  
* `na.value = `により `NA` の色を指定します。  
* `labels = `により<u>レジェンド中の表記</u>を指定します。  
* `name = `によりレジェンドタイトルを指定します。
...
ggplot(data = linelist)+
  ...
  scale_x_discrete(                   # 離散値のx軸（性別）
    expand = c(0,0),                  # パディングなし
    drop = FALSE,                     # 全てのファクターレベルを表示（データに存在しないものを含む）
...
ここでは、[コンタクトトレーシング](#contact-tracing-1)のページで扱った各症例とその感染源の年齢を値に持つデータを用いて、連続数に対応するカラースケールの調整方法を紹介します。 
...
* [`scales::rescale()`](https://www.rdocumentation.org/packages/scales/versions/0.4.1/topics/rescale) によって各色が勾配のどの位置に置かれるかを指定できます。この関数は数字ベクターをとり、0 から 1 の間で色の位置をリスケールします。 
...
`ggplot2` に慣れないうちは、図に現れる離散値の順序変更はしばしば難しく感じられるでしょう。しかし、一度仕組みを理解すれば簡単です。一般的に離散値は `factor` として扱われ、アルファベット順に並べられます。これを並べ替えるには、ファクター のレベルを表示したい順序に変更するだけです。`factor` の並べ替えについてのさらなる詳細は [ファクター](#factors)のページを参照してください。
...
ggplot で時間スケールを使用すると聞くと難しく感じるかもしれませんが、いくつかのキーになる関数を用いることでとても簡単に行うことができます。時間や日付を扱う際は、変数が正しくフォーマットされていることを確認してください。詳細は[日付の扱い](#working-with-dates-1)のページまたは前章 [ggplotの基礎](#ggplot-basics)内[流行曲線](#epidemic-curves)のページを参照してください。
...
**gghighlight** パッケージの `gghighlight()` 関数を用います。入力には論理分を用います – これにより様々な要素を強調した図を作成することができますが、ここではラインリストから年齢の分布を、臨床結果を強調しながら図示してみましょう。
...
**cowplot** パッケージは **ggplot2** と並行して使われます - **cowplot** を使うことで、**ggplot2** で作成した複数の図とレジェンドを 1 つにまとめて表示することができます。入力にRの **base** 関数で作成した図を用いることも可能です。 
...
図のファセット（[ggplot の基礎](#ggplot-basics)のページを参照）は便利な手法ではあるものの、表示方法に限りがあるため、望んだ図が得られないことがままあります。代わりに、複数の図を組み合わせて 1 つの図にしてしまうことも可能です。そのためのパッケージとして **cowplot**、**gridExtra**、**patchwork** の3つがあり、どれも大まかに同様の結果を得ることができます。ここでは **cowplot** に焦点を当て、使い方を見ていきましょう。
...
### レジェンドを組み合わせる {.unnumbered}  

もし複数の図が同じレジェンドを持つ場合、それらを統合することは比較的単純です。上記の例と同様 **cowplot** により図を組み合わせ、そのうち 1 つを残してレジェンドを取り除きます。
...
次に、レジェンドを統合した場合を見てみましょう。初めに<u>レジェンドなし</u>の図を作成し（`theme(legend.position = "none"`）、**cowplot** パッケージの `get_legend()` によりレジェンドを別に抽出します。レジェンドを抜き出す際には、`+` シンボルを用いて、配置（“right”）やタイトルも指定しながらレジェンドを戻し入れる必要があります。最後に2つのレジェンドを上下に結合し、先に作成した図と組み合わせて完成です！
...
* **cowplot** パッケージの `theme_half_open()` でメインの図を指定します；レジェンドは図の上部もしくは下部に配置すると良いでしょう。
* 埋め込む図を作成します。レジェンドの必要のない図を用いることが好ましいです。さらに以下のように、`element_blank()` を用いてプロットの `theme` 要素を削除することができます。
...
`ggplot2` を使用していて、1 つの図に 2 つの y 軸を表示したい場合があります。データの可視化において、そのような二重 y 軸の正当性は議論の的であり、またしばしば推奨されないものではありますが、あなたの上司がそれを求めてくるかもしれません。ここでは、それを実現する 1 つの方法を紹介します：**cowplot** を用いて、異なる y 軸を持つ 2 つの図を組み合わせます。

この方法では、2 つの独立した図を作成します – 1 つは y 軸を右側に、もう 1 つは左側に持つようにします。どちらも `theme_cowplot()` を使用して作成し、x 軸は同一でなくてはなりません。次に、作成した 2 つの図を重ね合わせます。これらは **cowplot** の持つ機能の内のほんの 1 つですが、[こちらのページ](https://wilkelab.org/cowplot/articles/aligning_plots.html)でさらに詳しく解説されています。
...
ちなみに [Epidemic curves](#epidemic-curves) のページでも別の例 - 累積症例数と流行曲線を組み合わせ - を示しています。
...
[データサイエンスのための R - ファクター](https://r4ds.had.co.nz/factors.html)  
