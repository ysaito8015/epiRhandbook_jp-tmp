...
前章 [ggplot の基礎](#ggplot-basics)の [mapping data to the plot](#ggplot_basics_mapping) のページで示した通り、`fill = ` や `color = ` などのエステティクスは `mapping = aes()` の*外側*と*内側*どちらからでも定義できます。`aes()` の外側で定義する場合、値は定数でなければならず（例 `color = "blue"`）、描画される全てのデータポイントに適応されます。内側から定義する場合、エステティクスは `color = hospital` のようにデータフレームの列に対応され、描画のされ方は各行の値に依存します。以下にいくつかの例を示します：  
...
# バープロットの fill を離散数からなる列、color を単一色で指定する
...
日付スケールについては [Epidemic curves](#epidemic-curves) のページでより詳しく解説しています。
...
* `labels = `によりレジェンド中の *表記* を指定します。  
* `name = `によりレジェンドタイトルを指定します。


以下の例では、まず棒グラフを初期設定で作成します。次に 3 つのスケール – 連続数のy軸、離散値のx軸、fill（バー内部の色指定）のマニュアル調整 – を変更したグラフを作成します。
...
ggplot(data = linelist)+
  ...
  scale_y_continuous(                # 連続数の y 軸（カウント）
    expand = c(0,0),                 # パディングなし
    breaks = seq(from = 0,
                 to = 3000,
                 by = 500))+
  
  scale_x_discrete(                   # 離散値の x 軸（性別）
    expand = c(0,0),                  # パディングなし
    drop = FALSE,                     # 全てのファクターレベルを表示（データに存在しないものを含む）
    na.translate = FALSE,             # NA を図から除外
    labels = c("Died", "Recovered"))+ # 表記の変更
    
  
  scale_fill_manual(                  # fill の色をマニュアル指定する（バー内部の色）
  ...
  labs(title = "Adjustment of scales") # fill レジェンドのタイトルを設定
```
