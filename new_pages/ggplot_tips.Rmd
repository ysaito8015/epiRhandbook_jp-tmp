...
**ggplot2** を使う上で最初に躓きやすいものが配色の調整です。この節では点や線、棒グラフやタイルなど、<u>図のオブジェクト</u>（geoms/shapes）の配色を扱うことに注意してください。テキストやタイトル、背景などの色の調整に関しては [ggplot の基礎](#ggplot-basics)のページの[テーマ](#ggplot_basics_themes)の節を参照してください。
...
列をエステティクスに対応させると（例 `x = `、`y = `、`fill = `、`color = `...）、 図にスケールと凡例が加わります。上記の例を見ると、スケールが連続数、離散数、日付など、スケールが対応する列のクラスによって変化する様子が分かると思います。列に対応するエステティクスが複数あるときは、スケールもその数だけ存在します。
...
ggplot(data = linelist)+
  ...
  scale_fill_manual(                  # fillの色をマニュアル指定する（バー内部の色）
    values = c("m" = "violetred",     # 色の指定
               "f" = "aquamarine"),
    labels = c("m" = "Male",          # 凡例の表記を変更（"="を使い間違いをなくす）
              "f" = "Female",
              "Missing"),
    name = "Gender",                  # 凡例タイトル
    na.value = "grey"                 # 欠測地の色を指定
  )+
  labs(title = "Adjustment of scales") # fill凡例のタイトルを設定
```
...
* `fill = `を `aes()` 内で設定することでラベルの色を指定できます。
  * その場合、凡例には文字 "a" が表示されますが、これは `guides(fill = guide_legend(override.aes = aes(color = NA)))` により除くことができます。
...
**gghighlight** パッケージの `gghighlight()` 関数を用いることでこの効果を得ることができます。この関数を利用するには、ロジカル型の結果となる構文を用います – これにより様々な要素を強調した図を作成することができますが、ここではラインリストから年齢の分布を、臨床結果を強調しながら図示してみましょう。
...
これは、グラフのファセットを作成する関数との相性も良いです – ファセットに用いた変数を図ごとに強調することができます！以下では週ごとの症例数を用いて流行曲線を作成し、病院ごとに分けて描画します（`color = `と `facet_wrap()` に `hospital` を設定）。
...
**cowplot** パッケージには多くの関数がありますが、そのうち最も使いやすいものが `plot_grid()` です。この関数は、複数のすでに作成された図を格子状に並べることができます。ここではマラリアデータセットを使って、地区毎の症例数、および時間毎の流行曲線を並べて図示する例を見ていきます。
...
もし複数の図が同じ凡例を持つ場合、それらを統合することは比較的単純です。上記の例と同様 **cowplot** により図を組み合わせ、そのうちの 1 つを残して凡例を取り除きます（重複を除きます）。

複数の図が異なる凡例を持つ場合は、別のアプローチを取ります：

1) `theme(legend.position = "none")` を用いて凡例を持たない図を作成する。
2) 以下の例のように `get_legend()` を用いて各図 – <u>判例を表示するよう変更したもの</u> - から凡例を抜き出す。
3) 抽出した判例を反例凡例パネルにまとめる。 
4) 図と凡例パネルを組み合わせる。


例として 2 つの図を別々に作成してからそれぞれの凡例を表示したまま格子状に（美しなく非効率的なスペースの使い方で）並べてみます：
...
次が 2 つのグラフが、凡例をまとめずに plot_grid() を利用した場合にどのような表現になるかを示します：
...
leg_p1 <- cowplot::get_legend(p1 +
                                theme(legend.position = "right",        # 凡例の抜き出し
                                      legend.justification = c(0,0.5))+ # 凡例の配列
                                labs(fill = "Outcome"))                 # 凡例タイトル
# p2（p2 + 凡例）から凡例を抜き出す
leg_p2 <- cowplot::get_legend(p2 + 
                                theme(legend.position = "right",         # 凡例の抜き出し
                                      legend.justification = c(0,0.5))+  # 凡例の配列
                                labs(fill = "Age Category"))             # 凡例タイトル

# 凡例配置用の空白図を作成
#blank_p <- patchwork::plot_spacer() + theme_void()

# 凡例パネルを作成。2つの凡例を上下に組み合わせる（もしくは上記コメント中のplot_spacer()を用いる）
...
上記のアプローチは[こちらのポスト](https://stackoverflow.com/questions/52060601/ggplot-multiple-legends-arrangement)を参照し、 凡例配置のための小さな修正を[こちら](https://github.com/wilkelab/cowplot/issues/33)を参考に加えています。


<span style="color: darkgreen;">**_コラム：_****cowplot** の "cow" は **cowplot** パッケージ製作者の名前 - Claus O. Wilke - から来ています。</span>  


### 図中に図を挿入する {.unnumbered} 
...
凡例  
[凡例の並べ替え](https://stackoverflow.com/questions/38425908/reverse-stacking-order-without-affecting-legend-order-in-ggplot2-bar-charts)  
