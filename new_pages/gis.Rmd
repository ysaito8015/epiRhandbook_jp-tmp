...
データを **rio** パッケージ（.xlsx, .csv, .rds など多くのファイルタイプを扱えます - 詳細は[データのインポート・エクスポート](#importing)の章を参照してください）の `import()` を使ってインポートします。 
...
この例では、`ggplot()` を `linelist_adm` で始めているので、頻度で棒グラフを並べる `fct_infreq()` などのファクタ関数を適用することができます（ヒントは [因子（ファクタ）型データ](#factors) の章を参照してください）。 
