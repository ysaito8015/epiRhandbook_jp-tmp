...
ggtree(tree, layout="circular", branch.length='none') %<+% sample_data + # %<+% の部分は、サンプルデータの入ったデータフレームをツリーに追加するためのものである
  aes(color=I(Belgium))+ # データフレーム内の変数に応じて枝に色をつける
...
scale_color_brewer(name = "Continent",  # name of your color scheme (will show up in the legend like this)
                       palette="Set1", # カラースキームの名前（凡例ではこのように表示される）
                   na.value="grey")+ # NAにはグレーを選択する
...
R の **base** パッケージ `head()` を使って最初の６つのエントリーを見ることで、 `tree` ファイルの `tip.labels` のフォーマットをチェックします。

```{r, phylogenetic_trees_inspect_sampledata}
head(tree$tip.label) 
```

また、`sample_data` データフレームの最初の列が `Sample_ID` であることを確認します。R の **base** パッケージ `colnames()` を使って、データフレームのカラム名を調べます。
...
```{r}
head(sample_data$Sample_ID) # もう一度、head() を使用して、初めの６つのデータだけ確認する
```
...
ggtree(tree, layout = "circular", branch.length = 'none') %<+% sample_data + # %<+% で、tree にサンプルデータを追加する
  aes(color = I(Belgium))+                       # データフレームの変数に応じて分枝に色付ける
  scale_color_manual(
...
また、１つの特定のサンプルに基づいてサブセットすることもできます。その際には、含める「後ろ」のノード数を指定します。系統樹の同じ部分を、サンプル（ここでは S17BD07692 ）に基づいて９ノードさかのぼってサブセットし、新しい `sub_tree2` オブジェクトとして保存します。
...
ggtree(sub_tree2) %<+% sample_data +     # %<+% 演算子を使ってsample_data にリンクしている
