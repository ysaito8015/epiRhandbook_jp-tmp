...
ggtree(tree, layout="circular", branch.length='none') %<+% sample_data + # %<+% の部分は、サンプルデータの入ったデータフレームをツリーに追加するためのものです
  aes(color=I(Belgium))+ # データフレーム内の変数に応じて枝に色をつけます
  scale_color_manual(name = "Sample Origin", # カラースキームの名前（凡例ではこのように表示されます
                    breaks = c("Yes", "No"), # 変数内の異なるオプション
                   labels = c("NRCSS Belgium", "Other"), # 凡例の中で異なるオプションをどのように命名するか、フォーマットを可能にします
                 values= c("blue", "black"), # 変数に割り当てる色
                 na.value = "black") + # NAにも黒色を割り当てます
  new_scale_color()+ # 別の変数の配色を追加することができます
     geom_tippoint(aes(color=Continent), size=1.5)+ # 先端の色を大陸ごとに変えていく場合は、"shape = ""を加えて形を変えることができます
scale_color_brewer(name = "Continent",  # name of your color scheme (will show up in the legend like this)
                       palette="Set1", # カラースキームの名前（凡例ではこのように表示されます）
                   na.value="grey")+ # NAにはグレーを選択しています
...
このページのデータは、\[Download handbook and data\] のページの説明に従ってダウンロードできます。
...
まず、**ape** パッケージの `read.tree()` 関数を使って、Newick の系統樹ファイルを .txt 形式で読み込み、クラス "phylo" のリストオブジェクトに格納しておきます。必要に応じて、**here** パッケージの `here()` 関数を使用して、相対ファイルパスを指定します。
...
**base** Rの `head()` を使って最初の６つのエントリーを見ることで、 `tree` ファイルの `tip.labels` のフォーマットをチェックします。

```{r, phylogenetic_trees_inspect_sampledata}
head(tree$tip.label) 
```

また、`sample_data` データフレームの最初の列が `Sample_ID` であることを確認します。**base** Rの `colnames()` を使って、データフレームのカラム名を調べます。
...
```{r}
head(sample_data$Sample_ID) # もう一度、head() を使用して、初めの６つのデータだけ確認します。
```

また、すべてのサンプルが `tree` ファイルに存在するかどうか、あるいはその逆かどうかを、一致する部分と一致しない部分に TRUE または FALSE の論理ベクトルを生成することで比較することができます。これらは、簡単にするために、ここでは表示しません。
...
ggtree(tree, layout = "circular", branch.length = 'none') %<+% sample_data + # %<+% で、tree にサンプルデータを追加する
  aes(color = I(Belgium))+                       # データフレームの変数に応じて分枝に色付けます
  scale_color_manual(
  ...
  new_scale_color()+                             # 別の変数の配色を追加します
    geom_tippoint(
      mapping = aes(color = Continent),          # 大陸別の先端の色。"shape = " を加えて形状を変更できます
      size = 1.5)+                               # 先端のポイントのサイズを定義します
  scale_color_brewer(
    name = "Continent",                    # カラースキームの名前（凡例ではこのように表示されます
    palette = "Set1",                      # brewer パッケージに付属しているカラーセットを選びます
    na.value = "grey") +                    # NA 値はグレーを選択
  geom_tiplab(                             # 枝の先端にサンプルの名前を追加 
    color = 'black',                       # (テキストラインは + で好きなだけ追加できますが、隣り合うように配置するにはオフセット値を調整する必要があります)
    offset = 1,
    size = 1,
    geom = "text",
    align = TRUE)+    
  ...
  theme(
    ...
    legend.title = element_text(    # 凡例のタイトルのフォントサイズとフォーマットを定義します
      face = "bold",
      size = 12),   
    legend.text=element_text(       # 凡例テキストのフォントサイズとフォーマットを定義します
      face = "bold",
      size = 10),  
    plot.title = element_text(      # プロットタイトルのフォントサイズとフォーマットを定義します
      size = 12,
      face = "bold"),  
    legend.position = "bottom",     # 凡例の配置を決めます
    legend.box = "vertical",        # 凡例の配置を決めます
...
p <- ggtree(tree,) %<+% sample_data +
  geom_tiplab(size = 1.5) +                # すべての枝の先端に、tree ファイルのサンプル名をラベル付けます
  geom_text2(
...
```{r}
p_collapsed + 
geom_point2(aes(subset = (node == 452)),  # 折り畳んだノードに記号を割り当てます
            size = 5,                     # 記号のサイズを定義します
            shape = 23,                   # 記号の形を定義します
            fill = "steelblue")           # 記号の塗りつぶしを定義します
```
...
ggtree(
  tree,
  branch.length = 'none',
  layout = 'circular') %<+% sample_data +               # %<+% 演算子を使って、サンプルデータを追加します
  geom_tiplab(size = 1)+                                # すべての枝の先端にサンプル名をラベル付けしてツリーファイルに保存
  geom_text2(
    mapping = aes(subset = !isTip, label = node),
    size = 3,
    color = "darkred") +                                # 系統樹内のすべてのノードにラベルを付けます
 theme(
...
sub_tree1 <- tree_subset(
  tree,
  node = 528)                                            # ノード 528 で系統樹をサブセットします
```
...
また、１つの特定のサンプルに基づいてサブセット化することもできます。その際には、含める「後ろ」のノード数を指定します。系統樹の同じ部分を、サンプル（ここでは S17BD07692 ）に基づいて９ノードさかのぼってサブセット化し、新しい `sub_tree2` オブジェクトとして保存します。

```{r}
sub_tree2 <- tree_subset(
  tree,
  "S17BD07692",
  levels_back = 9) # levels back は、先端（各サンプル）から何ノード後方に移動するかを定義します
```
...
p <- ggtree(sub_tree2) +  
  geom_tiplab(size = 4) +
  geom_text2(aes(subset=!isTip, label=node), # 系統樹内のすべてのノードにラベルを付けます
             size = 5,
             color = "darkred", 
             hjust = 1, 
             vjust = 1) 
...
p1 <- p + geom_hilight(  # ノード 39 を青くハイライトし、"extend ="でカラーブロックの長さを定義します。
  node = 39,
  fill = "steelblue",
  extend = 0.0017) +  
geom_hilight(            # ノード 37 を黄色くハイライトします
  node = 37,
  fill = "yellow",
  extend = 0.0017) +               
...
ggtree(sub_tree2) %<+% sample_data +     # %<+% 演算子を使ってsample_data にリンクしています
  geom_tiplab(                          # すべての枝の先端に、ツリーファイルのサンプル名をラベル付けします
    size = 2.5,
    offset = 0.001,
    align = TRUE) + 
  theme_tree2()+
  xlim(0, 0.015)+                       # 系統樹の x 軸の 下限、上限を設定
  geom_tippoint(aes(color=Country),     # 大陸別に先端を色付けます
                size = 1.5)+ 
  scale_color_brewer(
    name = "Country", 
    palette = "Set1", 
...
まず、系統樹をプロットし（これは線形でも円形でも構いません）、それを新しい ggtree plot のオブジェクト `p` に保存する必要があります。ここでは、パート3のsub_tree を使用します)。
...
h1 <-  gheatmap(p, gender,                                 # 性別データフレームのヒートマップレイヤーをツリープロットに追加します
                offset = 10,                               # offset は、ヒートマップを右方向に移動させます
                width = 0.10,                              # width は、ヒートマップのカラムの幅を定義します
                color = NULL,                              # color は、ヒートマップの列の境界線を定義します
         colnames = FALSE) +                               # ヒートマップのカラム名を隠します
...
注：WGS データにおける染色体の点変異の有無は、Zankari らが開発したPointFinder ツールを用いて事前に判断しています（参考文献の項を参照）。

まず、既存のプロットオブジェクト `h1` に新しい配色を割り当てて、今のオブジェクト `h2` に格納します。これにより、ヒートマップの２つ目の変数の色を定義・変更することができます。

```{r}
h2 <- h1 + new_scale_fill() 
```

次に、２つ目のヒートマップレイヤーを `h2` に追加し、合成したプロットを新しいオブジェクト `h3` に格納します。

```{r, phylogenetic_trees_sampledata_heatmap_cip_genes, out.width=c('80%'), fig.show='hold', fig.align='center'}

h3 <- gheatmap(h2, cipR,         # ヒートマップの２行目にCiprofloxacinの耐性変異を追加
               offset = 12, 
               width = 0.10, 
               colnames = FALSE) +
...
# そして、その２つを組み合わせて新しいプロットを作ります
h5 <- gheatmap(h4, MIC_Cip,  
               offset = 14, 
               width = 0.10,
                colnames = FALSE)+
  scale_fill_continuous(name = "MIC for Ciprofloxacin",  # ここでは、MIC の連続変数のグラデーションカラースキームを定義します
                      low = "yellow", high = "red",
