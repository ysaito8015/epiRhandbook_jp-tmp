...
* `slide_dbl()` - iterates through a numeric column performing an operation using a sliding window  
  * `slide_sum()` - rolling sum shortcut function  
  * `slide_mean()` - rolling average shortcut function  
* `slide_index_dbl()` - applies the rolling window using a separate *index* column (this is useful if using dates or if there are missing rows)  
...
Below, we show an example using `slide_index_dbl()` on the case linelist.  
...
Here is the `daily_counts` data frame - there are `r nrow(daily_counts)` rows, each day is represented by one row, but especially early in the epidemic *some days are not present (there were no cases admitted on those days)*.  
...
Let us say that our objective is to calculate a rolling 7-day incidence - the sum of cases using a rolling 7-day window. If you are looking for an example of rolling average, see the section below on [grouped rolling](#roll_slider_group).  
