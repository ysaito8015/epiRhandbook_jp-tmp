...
You can download the offline version of this handbook as an HTML file. Once downloaded, you can view the file in your web browser even if you no longer have internet access. If you are considering offline use of the Epi R Handbook here are a few things to consider:  
...
* Install our R package **handbookfiles** that contains all the example data (install process described below)  
...
We offer an R package called **handbookfiles**. It includes a function `download_book()` that will download the handbook file from our Github repository to your computer.  

This package also contains a function `get_data()` that will download all the example data to your computer.  


To install our R package **handbookfiles**, run the following code. This package is not on CRAN, so use the function `p_install_gh()` to install it from our *epirhandbook* Github page.   
...
Next, run the function `download_book()` (with empty parentheses) to download the handbook to your computer. A window will appear allowing you to select a save location.  
...
To "follow along" with the handbook pages, you can download the data and use them yourself.  

### Option 1: Use our R package {.unnumbered}  

The easiest approach to download all the data is to install our R package **handbookfiles**. It contains a function `get_data()` that will save all the example data to a folder of your choice on your computer.  
...
If you wish, you can also review all the data in this handbook in the **["data" folder](https://github.com/epirhandbook/Epi_R_handbook/tree/master/data)** of our Github repository.  



### Option 2: Download one-by-one {.unnumbered}  
...
If you follow Option 1 to download all the example data (via our R package **handbookfiles**), all the shapefiles are included.  
...
The page on [Dashboards with Shiny] demonstrated constructions of a simple app to display malaria data.  

To download the R files that produce the Shiny app:  

You can <a href='https://github.com/epirhandbook/Epi_R_handbook/blob/master/data/malaria_app/app.R' class='download-button'>
...
You can <a href='https://github.com/epirhandbook/Epi_R_handbook/blob/master/data/malaria_app/data/facility_count_data.rds' class='download-button'>
	click here to download the facility_count_data.rds file<span> that contains malaria data for the Shiny app. Note that you may need to store it within a "data" folder for the here() file paths to work correctly.</span></a>

You can <a href='https://github.com/epirhandbook/Epi_R_handbook/blob/master/data/malaria_app/global.R' class='download-button'>
	click here to download the global.R file<span> that should run prior to the app opening, as explained in the page.</span></a>
	
You can <a href='https://github.com/epirhandbook/Epi_R_handbook/blob/master/data/malaria_app/funcs/plot_epicurve.R' class='download-button'>
	click here to download the plot_epicurve.R file<span> that is sourced by global.R. Note that you may need to store it within a "funcs" folder for the here() file paths to work correctly.</span></a>
