...
-   列の値が所定の序列レベルになるように<u>因子</u>クラスに変換することを検討する。
...
キャプションの割り当てでは、**stringr** パッケージの `str_glue()` を使って、文字列テキストの中にダイナミックな R コードを埋め込んだことに注目してください。キャプションには、"Data as of:" キャプションには、linelist の最大入院日を反映した "Data as of: "の日付が表示されます。詳しくは、[文字列型データ](#characters-strings)の章をご覧ください。
...
カテゴリカルなデータには、文字値、論理値（TRUE ／ FALSE）、因子（[因子（ファクタ）型データ](#factors)の章を参照）などがあります。
...
#### 列のクラスと値の順序 {.unnumbered}

次に、プロットしたい列のクラスを調べます。ここでは、まず **base R** の `class()` を使って `hospital` を、そして **janitor**  パッケージの `tabyl()` を使って調べます。

```{r}
# hospital 列のクラスを確認する - それは文字列である
