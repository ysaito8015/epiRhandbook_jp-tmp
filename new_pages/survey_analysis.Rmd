...
```{r read_data_pop_show, eval = FALSE, eval=F}
...
```{r merge_data_levels, eval = FALSE, eval=F}
...
#
survey_data <- survey_data %>%
  drop_na(surv_weight_cluster_strata)
```
...
```{r descriptive_sampling_bias, warning = FALSE, eval=F}
...
```{r visualise_population_flow, eval=F}
