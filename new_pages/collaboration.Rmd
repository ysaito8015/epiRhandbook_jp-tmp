...
day-to-day use, you will rarely _need_ to interact with Git directly and interface usually provide nice visualisation tools for file modifications or branches. 

Many options exist, on all OS, from beginner friendly to more complex ones. Good options for beginners include the RStudio Git pane and [Github Desktop](https://desktop.github.com/), which we will showcase in this chapter.
Intermediate (more powerfull, but more complex) options include Source Tree, Gitkracken, Smart Git and others.
...
them, switch from one to another on a given project, use the console punctually for an action your interface does not support, or even perform any number of actions online on Github.*
...
This can happen sometimes: you attempt to pull but you have work that is not yet committed in your directory. 
If the files concerned by the remote changes and the files concerned by your local changes do not overlap, the pull will work.

There are however cases where Git will refuse to pull before you have done something with your modifications. There are several strategies, well described in [Happy Git with R](https://happygitwithr.com/pull-tricky.html), but they essentially boil down to:
- commit your changes, fetch changes, pull them in, resolve conflicts if needed and push everything online
- `stash` your changes, which sort of store them aside, pull, unstash (restore), and then commit, solve any conflicts, push... 
