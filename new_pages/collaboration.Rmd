...
More extensive tutorials, which informed the writing of this page can be
...
Google document, but for all types of files. It is one of the powerful
and most used options for version control.

**Why have I never heard of it? -** While people with a developer
background routinely learn to use version control softwares (Git,
Mercurial, Subversion or others), few of us are taught these skills in
more quantitative study tracks. Consequently, most epidemiologists never
...
However, it become even more powerful when used with a online repository
such as Github to support **collaborative projects**. This facilitates:

-   Collaboration so that others can review, comment on, and
    accept/decline changes

-   Publicly share your code, data, and outputs, and invite feedback
...
benefits of the tool. Learning a *small number of functions* can let you
track your changes, synchronize your files on a online repository and
collaborate with your colleagues in a very short amount of time.

Granted, epidemic emergency context may not be the best of time to learn
the tools, but learning can be achieved by steps: learning how to track
change locally on a small less urgent project project, how to create
branches, then how to synchronize with a remote directory. But once a
couple of notions are acquired, the workflow can be quite efficient and
fast.
...
While it is possible to merge two branches locally, or without informing
anybody (maybe one was a slight variation of your branch?), a merge may
be discussed, or checked by several people before being integraed to the
master branch. To help with the process, Github offers some discussion
functionalities around the merge: the **pull request**.

Typically, once you are done making changes and commmits, and you have
*pushed* the commits up to the remote Github repository, you can then
*request that your working branch be merged with the "main" branch*. You
could do that directly form the website (as illustrated bello), but also
similarly from Github Desktop.

-   Go to Github repository (online)
-   View the tab "Pull Requests" and click the "New pull request"
    button.\
...
When someone modified the same line(s) as you at the same time, a merge
conflict will arise. Indeed, Git will refuse to make a decision about
which version to keep instead of you (and it is wise), but it will help
you find the exact place of the conflict. Most of the time, it is pretty
straighforward to resolve. **DO NO PANIC**.
...
You need to decide about which version of the code you prefer (or to do
a third, including changes from both sides if pertinent), write it and
then remove all the marks that Git added *(\<\<\<\<\<\<\< HEAD, =======,
\>\>\>\>\>\>\> origin/master/your_branch_name*). Then, save the file,
stage it and commit it.

Once the conflicts have been resolve, you will need a last commit to
...
main\^ is the parent commit of main git checkout main\^ = this moves the
HEAD to the commit before main's last commit HEAD can be used as a
