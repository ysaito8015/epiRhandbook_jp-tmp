...
One can merge branches locally using Github Desktop. First, go (checkout) to the branch that will be the recipient of the commits, the branch you want to update. Then go to the menu _Branch > Merge into current branch_ and click. A box will allow you to select the branch you want to "import" from.
...
While it is possible to merge two branches locally, or without informing anybody (maybe one was a slight variation of your branch?), a merge may be discussed, or checked by several people before being integraed to the master branch. To help with the process, Github offers some discussion functionalities around the merge: the **pull request**. 

Typically, once you are done making changes and commmits, and you have *pushed* the commits up to the remote Github repository, you can then _request that your working branch be merged with the "main" branch_. You could do that directly form the website (as illustrated bello), but also similarly from Github Desktop.
...
When someone modified the same line(s) as you at the same time, a merge conflict will arise. Indeed, Git will refuse to make a decision about which version to keep instead of you (and it is wise), but it will help you find the exact place of the conflict. Most of the time, it is pretty straighforward to resolve. **DO NO PANIC**.
...
As usual, you can use different tools. You can do some edits directly from Github for very minor things. But you can also use your favourite code writing software.

After the merge raised a conflict, open the file. The conflict will be indicated by series of characters:

```{r echo=F, out.width = '100%', out.height='200%', fig.align = "center"}
knitr::include_graphics(here::here("images", "github_conflict3.png"))
```

The text between _<<<<<<< HEAD_ and _=======_ is the one on your local repository, and the one between _=======_ and _>>>>>>>_ the one on the other branch (which may be origin, master or any branch of your choice).

You need to decide about which version of the code you prefer (or to do a third, including changes from both sides if pertinent), write it and then remove all the marks that Git added _(<<<<<<< HEAD, =======, >>>>>>> origin/master/your_branch_name_). Then, save the file, stage it and commit it.

Once the conflicts have been resolve, you will need a last commit to officialize the merging.

The more often you and your collaborators pull and push, the smaller the conflicts will be.

### Delete your branch {-}  

Once a branch was merged into master and is no longer needed, you can suppress it.
...
Just go checkout to the branch you want to delete, and go to the menu _Branch > Delete_.
