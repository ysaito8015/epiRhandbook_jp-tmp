...
can let you track your changes, synchronize your files on a online
...
Console) or the Git Bash for Windows Studio "Terminal".

To learn Git commands see this online , or the later section on
[Git](#git). .
...
sections, we will show how to use work using interfaces, but it is good
to have the vocabulary in mind, at you encounter most of it in
interfaces anyway.
...
A *commit* is a **snapshot** of the project currently staged changes. In
other words, it is trackable unit of changes (the delta) made to your
...
and coherent. You will also attach a brief description of the changes as
the "commit message".

Staged changes? To stage changes is to add them to the *staging area* to
prepare them for the next commit. The idea is that you can finely decide
which changes to include in a given commit.

For example, if you worked on model specification in one script, and
later on a figure in another script, it would make sense to have two
different commits (if you want to revert the changes made on the figure
but not the model, this would be easier).
...
your files. When you are done experimenting on a branch, you can bring
the changes into your *main* branch, by *merging* it, or suppress it, if
...
or vice-versa. You get to choose when to synchronise.
...
-   When you have made one or several commits locally, you can
    `git push` the commits to the remote repository. This will send your
...
Creating a branch in Git is easy and fast. This encourage a workflow
based on creating a branch, make some modifications, some tests, and
then to merge it back in the main branch (or another branch), or delete
it if the changes did not work out so well.

When you select a branch to work on, **Git resets your working directory
the way it was the list time you were on this branch**.
...
modifications related to a common purpose, and to bundle together
...
anybody, a merge may be discussed, or investigated by several people
before being integrated to the master branch. To help with the process,
Github offers some discussion features around the merge: the **pull
request**.

Typically, once you are done making changes and commits, and you have
*pushed* the commits up to the remote Github repository, you can then
*request that your working branch be pulled into the "main" branch*. You
could do that directly form the website (as illustrated bellow) or from
Github Desktop.
...
conflict. Most of the time, it is pretty straighforward to resolve. **DO
NO PANIC**.
