...
Sometimes, you commit  some changes, carry on working, and realise
that you made changes that conceptually belong to the past commit. 
...
an "UNDO" button appears just unde the COMMIT button. If you click on it, 
it will revert your commit (but let your files staged and keep your commit message). 
...
to push when you are done with introducing change on the particular point that 
you were working on.
...
This can happen sometimes: you attempt to pull but you have work that is 
not yet committed in your directory. 
If the files concerned by the remote changes and the files concerned 
by your local changes do not overlap, the pull will work.

There are however cases where Git will refuse to pull before you have 
done something with your modifications. There are several strategies, 
well described in [Happy Git with R](https://happygitwithr.com/pull-tricky.html), 
but they essentially boil down to:
- commit your changes, fetch changes, pull them in, resolve conflicts 
if needed and push everything online
- `stash` your changes, which sort of store them aside, pull, unstash 
(restore), and then commit, solve any conflicts, push... 
...
A pull request typically involves multiple commits, which each contain changes 
to certain files. A pull request usually begins a conversation and review 
process before the it is accepted and the branch is merged. For example, 
see pull request discussions on the [dplyr's
...
You can fork a project if you would like to contribute to a project but 
do not have the rights to create a branch on this project. Or if you just 
want to make some modifications to a project for your personnal use. A 
...
there are two version of the repository **on Github**: the original one,
...
on your computer, using any of the methods describes in previous sections. 
Then, you can create a new branch, make changes, commit and push them 
_to your remote repository_.

Once you are happy with the result, if you wish so, you can create a Pull Request 
...
It is possible to synchronize your fork with the original folder. 
It involves using the terminal, but it should not too complicated. 
...
see two lines, begining by _origin_. These shows the remote repo 
...
Here the adress is the adress that Github generates when you clone
...
in the Resolving conflicts session. 


**Summary**: forking is cloning, but on the Github server side. 
The rest of the actions as typical collaboration workflow actions 
...
One of the advantages of version control is that you can see the past.
Maybe there is this beautiful graph that you did three months ago but
then deleted which is suddenly needed again. You can go in the history of commits
...
- commit changes, 
- synchronise your local and remote repositories.

You also saw how to visualize the state of your files in the past.

All these should get you going and be enough for most of your needs as 
epidemiologists. We usually do not have as advanced usage as fullstack developpers. 

However, know that should you want (or need) to go further, Git offers more power to simplify 
commit histories, revert one or several commits, cherry-picking commits etc. 
Some of it may sound like pure wizardry, but now that you have the basic, 
it is only a matter of adding new layers of understanding and power to your knowledge.
...
you can very easilly install an interface to try it out sometimes, 
or to perform some less common complex task in it occasionnaly, 
...
[
]More beginner-friendly informations on Git Bash](https://happygitwithr.com/shell.html), how to find it and some bash commands you will need.
...
| `git commit -m <message>`| Commit currently saved and staged changes to current branch with message |
...
The official [Pro Git book](https://git-scm.com/book/en/v2) is considered a reference. 
