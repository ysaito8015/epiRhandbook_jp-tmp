...
all tracking information. One of your typical Git repository will be
...
terminal in Rstudio) and to type `git init` but will show other, easier
ways to do that from Github, Github Desktop or Rstudio in the next
sections.
...
to prepare them for the next commit. The idea is that you can finely
...
another script, it would make sense to have two different commits (if
you want to revert the changes made on the figure but not the model,
...
*Note: you do not have to calloborate with other people to use branches,
nor need to have a remote repository online.*
...
your computer or the contrary, begin with a local repository and clone
it online on Github.
...
or vice-versa. You get to choose whe and how to synchronise.

-   `Git fetch` verifies the state of the remote repository.

-   `git pull` downloads the new changes from the remote repositories
    into your local bhanch.
...
project at the same time (see instruction above).
